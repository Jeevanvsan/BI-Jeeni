...
class doc_generator_overview:
    ...
    def run(self):
        ...
        for file in self.data:
            self.file = file['file']
            h1 = doc.add_heading(file['file'],1)
            # h1.add_instrText(rf'HYPERLINK \l "{bkm_name}"')
            title_format = h1.runs[0].font
            title_format.size = Pt(54)
            h1.alignment = WD_ALIGN_PARAGRAPH.CENTER
            h1.bold = True
            self.schema(file['meta'])
            doc.add_page_break()
            self.visual_doc(file['meta'],file['file'])
            doc.add_page_break()

        doc.save(f'output/BIx Overview.docx')
    ...
    def front_page(self):
        ...
        h1 = doc.add_heading('Files',1)
        title_format = h1.runs[0].font
        title_format.size = Pt(30)
        h1.alignment = WD_ALIGN_PARAGRAPH.CENTER
        h1.bold = True
        for file in self.data:
            h1 = doc.add_heading(file['file'],2)
            title_format = h1.runs[0].font
    ...
    def schema(self,data):
        ...
        for tables in data['schema']['tables']:
    ...
    def visual_doc(self,data,file_name):
        doc = self.doc
        for i,report in enumerate(data['layout'],1):
            ...
            h2.bold = True
            doc.add_page_break()
            self.layout_doc(report['visualContainers'],file_name,report)
            doc.add_page_break()
    ...
    def layout_doc(self,report,file_name,main_report):
        doc = self.doc
        ...
        for i,visual in enumerate(report,1):
            for container_ in visual:
                ...
                if 'image_url' in container:
                    ...
                    doc.add_picture(f'output/{file_name}/img/Report/{url_}',width=Inches(2))
